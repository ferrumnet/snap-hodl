...
export interface ITradingVolumeContractData {
    ...
    toBlock: { type: string | number, validate: { validator: (value: any) => boolean, message: string }, required: boolean };
    blockIterationSize: { type: number; min: number; required: boolean };
    liquidityPoolAddress: { type: string, match: RegExp, validate: { validator: (value: any) => boolean, message: string }, required: boolean };
  }
...
export const tradingVolumeContractDataSchema = new Schema<ITradingVolumeContractData>({
    _id: false,
    ...
    tradingPoolType: { type: String, enum: ['standard', 'open'], required: true },
    tokenContractAddress: {
        type: String,
        required: true,
        validate: {
